...
> 8. 作った件数、`rejected`（＝既にあった枠）の件数、新しい最遠日を報告する
...
- **`datetime` は「壁の時計」であって「瞬間」ではない。** `YYYY-MM-DDTHH:MM`（秒は任意、
  **タイムゾーンの接尾辞は付けない**）。`new Date(...).toISOString()` が反射的に出てきますが、
  ...
  形式が通っていたら、全枠 7 時間ずれたまま公開されていました。文字列は部品から組み立てること
  (`` `${dateKey}T${hh}:00` ``)。なお `opensAt` / `closesAt` は別物で、epoch millis の
  **number** です。
  そして `putItems` はこれを**受理します** — 型の形を見るのは読み取り側と publish なので、
  1 日分を書いて `getItems` で読み返し、`check` を通してから残りを生成すること
...
- **再実行は `state` については安全です。** `mirrorOf` のルールが「予約があるなら taken、
  ...
  衝突した行は `rejected` に入って返るので、これは失敗ではなく「既にあった枠」として数えます
