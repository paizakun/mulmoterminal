...
export const MANAGE_SHARED_APP: ToolDefinition = {
  ...
  prompt:
    ...
    "**preview** RUNS the pages. It loads each one in a real headless browser, inside the same sandbox and CSP a visitor gets, hands it the app's real records, and presses each control on a freshly loaded copy of the page. It runs to a budget and SAYS what it left out, so read the counts rather than assuming everything was covered. It reports what a person would otherwise have to notice by eye: a page that never finished loading, a button that does nothing, a form the sandbox blocked, a submission the declaration refused. It writes NOTHING today: accepting a submission means putting a real record in a real app, that needs proof the CLICK caused it, and the installed runtime does not mark that yet — so the report tells you a submission REACHED the parent, and not what the deployed rules would say about it. For that, ask the user to press Preview in the Collections pane, where a person supplies the proof. It also writes a picture of each page and gives you the path; open it when the words leave the layout in doubt. Run it after writing or editing any view, and again before you publish. If it cannot start a browser it says so; then ask the user to press Preview in the Collections pane.\n" +
