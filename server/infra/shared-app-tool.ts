...
export const MANAGE_SHARED_APP: ToolDefinition = {
  ...
  description:
    ...
    "publish writes the declaration, the collection schemas and the app's pages, and opens the app to the public; unpublish closes it again.",
  prompt:
    ...
    "**publish** is the dangerous one, and it is the ONLY thing that writes an app after `init`. It writes this repository's declaration, schemas and pages as they are right now, and then opens the app — so run `preview` first: nothing else stands between what an LLM wrote and what everybody sees. Publish only when the user asks for it in those terms.\n" +
    "**unpublish** closes the app to the public and leaves the schemas and pages in place, so the roster goes on using it and publishing again just re-opens it.\n" +
