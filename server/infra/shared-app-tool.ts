...
export const MANAGE_SHARED_APP: ToolDefinition = {
  ...
  prompt:
    ...
    "**preview** RUNS the pages. It loads each one in a real headless browser, inside the same sandbox and CSP a visitor gets, hands it the app's real records, and presses each control on a freshly loaded copy of the page. It runs to a budget and SAYS what it left out, so read the counts rather than assuming everything was covered. It reports what a person would otherwise have to notice by eye: a page that never finished loading, a button that does nothing, a form the sandbox blocked, a submission the declaration refused. It ACCEPTS what a page submits: the record is written to the real database as the user, judged by the deployed rules, and REMOVED again immediately — so the report tells you whether a visitor's submission would actually be accepted, and names any record it could not take back. It also writes a picture of each page and gives you the path; open it when the words leave the layout in doubt. Run it after writing or editing any view, and again before you publish. If it cannot start a browser it says so; then ask the user to press Preview in the Collections pane.\n" +
