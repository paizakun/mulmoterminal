...
### D-2c. 「押したから submit された」は証明できない。証明できないなら書かない

**レビューで 3 巡かかった場所**（codex P1 ×3）。押下の前後で件数を数えても、増えたのが
**クリックのせい**だとは言えない:

1. load 時に上がった確認をそのまま受諾していた（`pending` は 1 つのセル）
2. 押す前に decline したら、ページがその promise の `false` 分岐から即 submit し直し、
   それがクリックの成果に数えられた
3. 1 と 2 を直しても、**タイマーで 800ms 後に submit するページ**は防げない。押下窓の中に
   たまたま着いたものと、押下が起こしたものは、親の側から区別できない

3 は原理的に区別できない。なので**数え方を直すのをやめて、書く条件を変えた**:

> ページを `unpromptedMs` のあいだ**何もせずに見る**。その間に 1 回でも自分から submit した
> ページでは、**このページでは一切書かない**。

破壊的な操作は、原因が分からないなら**やらない**側に倒す。これは上限（D-3）とは別の理由で
書かないので、報告のフラグも別（`writeWithheld` と `writeSkipped`）。

そして**測定は窓であって証明ではない**ので、報告にその窓の長さを書く — 「これより遅い
タイマーのページは静かに見える」。読み手が保証と取り違えないために。
