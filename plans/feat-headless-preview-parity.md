...
### D-2c. 「押したから submit された」は証明できない。証明はランタイムから受け取る

**レビューで 4 巡かかった場所**（codex P1 ×4）。押下の前後で件数を数えても、増えたのが
**クリックのせい**だとは言えない。順に潰そうとして、順に破られた:

1. load 時に上がった確認をそのまま受諾していた（`pending` は 1 つのセル）
2. 押す前に decline したら、ページがその promise の `false` 分岐から即 submit し直し、
   それがクリックの成果に数えられた
3. 押す前に `unpromptedMs` ページを観察して、自分から submit するページでは書かない
   → **その窓より後に submit するタイマー**には効かない

3 で分かったのは、窓の長さの問題ではないということ。**経過時間から因果は出ない**ので、
どこに線を引いてもその外側がある。

なので判断をやめて、**事実を受け取る**ことにした:

> 書き込むのは、**ランタイムが「クリックの処理中に呼ばれた submit だ」と印を付けたもの**だけ。
> 印が無ければ書かない。

印を付けられるのは、ページと同じ realm にいる**注入されたブートストラップ**
（`@receptron/sharedapp`）だけである。そこはイベントのディスパッチの中にいるので、
「この submit は click の処理中に呼ばれたか」を**事実として**知っている。タイマーの
コールバックは別のタスクなので、印は付かない。時間の話が一切出てこない。

ハーネスは印を**生のメッセージから**読む（`requestId` で対応付ける）。親の
